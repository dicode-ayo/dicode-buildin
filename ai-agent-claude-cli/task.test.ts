...
exit 1`,
  ...
  if (matches.length < 2) {
    throw new Error(
      `expected 2 <redacted> placeholders, got ${matches.length} in: ${err}`,
    );
  }
});
...
JSON`,
  ...
  if (String(result.error ?? "").includes("supersecret-token-xyz")) {
    throw new Error(
      `OAuth token leaked via the is_error envelope: ${result.error}`,
    );
  }
  if (!String(result.error ?? "").includes("<redacted>")) {
    throw new Error(`expected <redacted> placeholder, got ${result.error}`);
...
Deno.test("redacts token in JSON-parse-failure path too", async () => {
  ...
  if (String(result.error ?? "").includes("supersecret-token-xyz")) {
    throw new Error(
      `OAuth token leaked via stdout/JSON-parse path: ${result.error}`,
    );
  }
});
...
JSON`,
  ...
  if (
    !cfg.mcpServers?.dicode?.headers?.Authorization?.includes(
      "dck_test_mcp_key",
    )
  ) {
    throw new Error(
      `expected dicode MCP key in .claude/mcp.json, got ${recorded}`,
    );
  }
...
JSON`,
  ...
  if (listed.includes("passwd")) {
    throw new Error(
      `path-traversal slipped through; .claude/skills lists: ${listed}`,
    );
  }
});
...
Deno.test("buildClaudeArgs: mounts MCP strictly + explicitly with a config path", () => {
  const a = buildClaudeArgs({
    prompt: "hi",
    mcpConfigPath: "/w/.claude/mcp.json",
  });
...
Deno.test("buildClaudeArgs: allowlists the dicode MCP tools when MCP is wired", () => {
  ...
  assertEquals(
    buildClaudeArgs({ prompt: "hi" }).includes("--allowedTools"),
    false,
  );
});
...
JSON`,
  ...
  if (!recorded.includes("--strict-mcp-config")) {
    throw new Error(
      `expected --strict-mcp-config in claude args, got:\n${recorded}`,
    );
  }
  ...
  if (i < 0 || !lines[i + 1]?.endsWith("/.claude/mcp.json")) {
    throw new Error(
      `expected --mcp-config <…/.claude/mcp.json>, got:\n${recorded}`,
    );
  }
  Deno.env.delete("DICODE_MCP_API_KEY");
});
...
JSON`,
    async () => {
      try {
        await steps.turn({
          ...
          input: { message: "again" },
          state: {
            claudeSessionId: VALID_PRIOR_SESSION_ID,
            chatId: VALID_CHAT_ID,
          },
          dicode,
          output: {} as unknown as DicodeSdk["output"],
  ...
  if (i < 0 || lines[i + 1] !== VALID_PRIOR_SESSION_ID) {
    throw new Error(
      `expected --resume ${VALID_PRIOR_SESSION_ID}, got:\n${lines.join(" ")}`,
    );
  }
...
JSON`,
  ...
  if (recordedCwd.includes("dicode-chatid-escape")) {
    throw new Error(
      `chatId path traversal reached the subprocess cwd: ${recordedCwd}`,
    );
  }
  const basename = recordedCwd.split("/").pop() ?? "";
  if (!isValidSessionId(basename)) {
    throw new Error(
      `expected a fresh UUID-shaped workdir, got cwd: ${recordedCwd}`,
    );
  }
});
...
JSON`,
  ...
  if (args.includes("--resume")) {
    throw new Error(
      `expected --resume to be omitted for an invalid carried claudeSessionId, got:\n${
        args.join(" ")
      }`,
    );
  }
  if (args.includes("--dangerously-skip-permissions")) {
    throw new Error(
      `malicious claudeSessionId leaked into claude args:\n${args.join(" ")}`,
    );
  }
});
...
JSON`,
  ...
  if (args.includes("--resume")) {
    throw new Error(
      `expected --resume to be omitted when chatId had to be replaced, got:\n${
        args.join(" ")
      }`,
    );
  }
...
Deno.test("skillLoadError: rejects missing frontmatter, name, and description", () => {
  assertEquals(
    skillLoadError("legit-skill", "# no frontmatter\n"),
...
Deno.test("skillLoadError: rejects a name that disagrees with the directory", () => {
  const err = skillLoadError(
    "legit-skill",
    SKILL_DOC.replace("name: legit-skill", "name: other"),
  );
  if (!err.includes("does not match")) {
    throw new Error(
      `expected a name-mismatch error, got ${JSON.stringify(err)}`,
    );
  }
});
...
Deno.test("skillLoadError: accepts a quoted name (the CLI reads YAML, not raw text)", () => {
  assertEquals(
    skillLoadError(
      "legit-skill",
      SKILL_DOC.replace("name: legit-skill", 'name: "legit-skill"'),
    ),
    "",
  );
});
...
JSON`,
  ...
  if (!invoking.includes("skills=1")) {
    throw new Error(
      `expected skills=1 for a name given twice, got: ${invoking}`,
    );
  }
});
