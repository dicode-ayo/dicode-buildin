...
export default async function main({ params, dicode, output }: SDK) {
  ...
  if (decideEntryMode(prompt) === "chat-start") {
    ...
    await chatStart(
      ...
      "Message Claude — leave blank to end.",
    );
  ...
  turn(ctx: DicodeSdk) {
    ...
    return chatTurn(
      ...
      async ({ message, state }) => {
        ...
        const hasValidChatId = !!carried.chatId &&
          isValidSessionId(carried.chatId);
        const chatId = resolveSessionId(
          carried.chatId,
          "ai-agent-claude-cli: chatId",
          { autoMint: true },
        );
        ...
        const priorClaudeSessionId = hasValidChatId
          ? resolveSessionId(
            carried.claudeSessionId,
            "ai-agent-claude-cli: claudeSessionId",
            { autoMint: false },
          )
          : "";
      ...
      (state) => ({
        ...
        session_id: resolveSessionId(
          (state as ClaudeChatState | undefined)?.claudeSessionId,
          "ai-agent-claude-cli: claudeSessionId(end)",
          { autoMint: false },
        ),
      }),
    );
...
export function skillLoadError(name: string, doc: string): string {
  ...
  if (declared !== name) {
    ...
    } does not match the skill name ${JSON.stringify(name)}`;
  }
...
async function runClaudeTurn(opts: {
  ...
  if (!isValidSessionId(workdirKey)) {
    return fail(
      output,
      `internal error: workdirKey is not a valid session id — refusing to build a workdir path from it`,
    );
  }
  if (priorClaudeSessionId && !isValidSessionId(priorClaudeSessionId)) {
    return fail(
      output,
      `internal error: priorClaudeSessionId is not a valid session id — refusing to pass it to --resume`,
    );
  }
  ...
  const enableMcp =
    ((await params.get("enable_mcp")) ?? "true").toLowerCase() !== "false";
  ...
  if (!cliPath) {
    return fail(
      output,
      "claude binary not found. Set the cli_path param, or install via one of the paths in ./ai-agent-claude-cli/README.md.",
    );
  }
  ...
  if (!workdirBase) {
    return fail(
      output,
      "workdir_base param is empty; expected the default ${DATADIR}/tmp/claude-cli to resolve at spec-load time",
    );
  }
  ...
  try {
    ...
    if (skillsParam && skillsDir) {
      const names = [
        ...new Set(skillsParam.split(",").map((s) => s.trim()).filter(Boolean)),
      ];
      for (const name of names) {
        if (!SAFE_SKILL_NAME.test(name)) {
          console.warn(
            `ai-agent-claude-cli: skipping skill ${
              JSON.stringify(name)
            } (invalid name)`,
          );
          continue;
        ...
        try {
          ...
          if (problem) {
            console.warn(
              `ai-agent-claude-cli: skipping skill ${name}: ${problem}`,
            );
            continue;
          }
          ...
          console.warn(
            `ai-agent-claude-cli: skipping skill ${name}: ${
              e instanceof Error ? e.message : String(e)
            }`,
          );
        }
    ...
    return fail(
      output,
      `workdir setup failed at ${workdir}: ${
        e instanceof Error ? e.message : String(e)
      }`,
    );
  }
  ...
  console.log(
    `ai-agent-claude-cli: invoking ${cliPath} (resume=${
      priorClaudeSessionId ? priorClaudeSessionId.slice(0, 8) + "…" : "no"
    }, model=${model || "default"}, workdir=${workdirKey.slice(0, 8)}…, mcp=${
      mcpWired ? "on" : "off"
    }, skills=${skillsWired})`,
  );
...
async function runClaudeAndDecode(
  ...
  if (!out.success) {
    ...
    return fail(
      output,
      `claude exited ${out.code}: ${redact(stderr) || "(no stderr)"}`,
    );
  }
  ...
  try {
    ...
    return fail(
      output,
      `claude returned non-JSON output: ${redact(stdout.slice(0, 500))}`,
    );
  }
  ...
  console.log(
    `ai-agent-claude-cli: ok (model=${parsed.model ?? "?"}, cost=$${
