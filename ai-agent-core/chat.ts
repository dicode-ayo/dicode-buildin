...
export function resolveSessionId(
  ...
  if (id && !isValidSessionId(id)) {
    console.warn(
      `${label}: rejected invalid session id; ${
        opts.autoMint ? "minting a fresh one" : "treating as absent"
      }`,
    );
...
export type RunTurn = (
  args: { message: string; state: unknown },
) => Promise<TurnResult>;
...
export async function chatTurn(
  ...
  const message = String(
    (ctx.input as { message?: string } | undefined)?.message ?? "",
  );
  ...
  if (!turn.ok) {
    ...
    const reason = typeof turn.error === "string"
      ? turn.error
      : "chat turn failed";
