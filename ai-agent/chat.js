(function () {
  ...
  const STORAGE_KEY = "dicode-ai-session";
  ...
  const pendingSetups = Object.create(null); // secretName → retryFn
  const SETUP_TTL_MS = 15 * 60 * 1000; // abandon stale registrations after 15 min

  // BroadcastChannel is inherently same-origin, so no origin check needed.
  try {
    const _ch = new BroadcastChannel("dicode-secrets");
  ...
  // deno-lint-ignore no-control-regex -- \u001b is the escape being stripped
  const ANSI_RE = /\u001b\[[\d;]*m/g;
  ...
  function detectSetup(body) {
    ...
    const m = errStr.match(
      /if_missing: secret "([^"]+)" requires setup via task "([^"]+)"/,
    );
    if (!m) return null;
    const secret = m[1];
    const taskId = m[2];

    const logBlob = Array.isArray(data.logs) ? data.logs.join("\n") : "";
    const urlMatch = stripAnsi(logBlob).match(/https?:\/\/[^\s]+/);
    if (!urlMatch) return null;

    const slug = taskId.replace(/^auth\//, "").replace(/-oauth$/, "");
    const providerName = PROVIDER_DISPLAY_NAMES[slug] ||
  ...
  function renderSetupCard(el, info, retryFn) {
    ...
    $desc.textContent = "Authorize " + info.providerName +
    ...
    const prev = pendingSetups[info.secret];
  ...
  function send(text) {
    ...
    dicode.execute(callParams).then(function (result) {
      ...
      if (result.status !== "success" || !payload) {
        const body = result.body || "unknown error";
        const setup = detectSetup(body);
        if (setup) {
      ...
      renderError(
        pending,
        "Connection error \u2014 is dicode running? (" + e.message + ")",
      );
    }).then(function () {
      $send.disabled = false;
      $prompt.value = "";
