...
export default async function main(opts: { params: Map<string, string> }) {
  ...
  if (!SOURCE_ID_RE.test(sourceID)) {
    return {
      ...
      } contains invalid characters; expected ^[A-Za-z0-9_][A-Za-z0-9_-]{0,63}$`,
    };
  }
  ...
  if (!ghToken) {
    return {
      ok: false,
      error:
        "GH_TOKEN is not set; refusing to invoke gh with ambient credentials. Configure the GH_TOKEN_AUTOFIX secret.",
    };
  }
  ...
  if (!clonePath) {
    return {
      ok: false,
      error:
        "clone_path is required; pass the path returned by dicode.sources.set_dev_mode",
    };
  }
  ...
  if (!clonePath.startsWith(cloneRoot + "/") && clonePath !== cloneRoot) {
    return {
      ok: false,
      error: `clone_path must be rooted at ${cloneRoot}; got ${clonePath}`,
    };
  }
