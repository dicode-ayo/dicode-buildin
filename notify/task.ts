...
export default async function main({ params }: DicodeSdk) {
  ...
  if (os === "linux") {
    ...
    const which = await new Deno.Command("which", { args: ["notify-send"] })
      .output();
    if (!which.success) {
      throw new Error(
    ...
    cmd = [
      "osascript",
      "-e",
      `display notification "${escaped(displayBody)}" with title "${
        escaped(title)
      }"`,
    ];
  } else if (os === "windows") {
  ...
  const result = await new Deno.Command(cmd[0], { args: cmd.slice(1) })
    .output();
  if (!result.success) {
