...
class DcConfig extends LitElement {
  createRenderRoot() {
    return this;
  } // Monaco needs direct DOM access
  ...
  async _saveAI() {
    ...
    try {
      ...
      if (!task) {
        this._aiStatus = "Enter a task id";
        return;
      }
      await post("/api/settings/ai", { task });
      this._aiStatus = "Saved ✓";
      setTimeout(() => {
        this._aiStatus = "";
      }, 2000);
      this._load();
    } catch (e) {
      this._aiStatus = "Error: " + e.message;
    }
  }
  ...
  async _saveServer() {
    ...
    try {
      ...
      this._srvStatus = "Saved ✓";
      setTimeout(() => {
        this._srvStatus = "";
      }, 2000);
    } catch (e) {
      this._srvStatus = "Error: " + e.message;
    }
  }
  ...
  async _removeSource(name) {
    ...
    try {
      await del(`/api/settings/sources/${name}`);
      this._load();
    } catch (e) {
      this._srcStatus = "Error: " + e.message;
    }
  }
  ...
  async _addSource() {
    ...
    if (!name) {
      this._srcStatus = "Error: name is required";
      return;
    }
    if (!SOURCE_NAME_RE.test(name)) {
      this._srcStatus =
        "Error: name must start with a letter or digit and contain only letters, digits, '-', '_', or '.' (max 64 characters)";
      return;
    }
  ...
  async _saveRaw() {
    ...
    try {
      ...
      this._cfgStatus = "Saved ✓";
      setTimeout(() => {
        this._cfgStatus = "";
      }, 2000);
    } catch (e) {
      this._cfgStatus = "Error: " + e.message;
    }
  }
  ...
  render() {
    if (this._error) {
      return html`
        <p style="color:red">Error: ${this._error}</p>
      `;
    }
    if (!this._cfg) {
      return html`
    ...
    const tray = srv.Tray != null
      ? srv.Tray
      : (srv.tray != null ? srv.tray : true);
    ...
    return html`
      ...
      <div class="card">
        <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:0.75rem">
          ...
          <span style="font-size:0.82rem;color:var(--dicode-muted)">${this
        ._aiStatus}</span>
        </div>
        <div class="cfg-form">
          <div class="field">
            ...
            <div class="hint">
              Task id that powers both the task-detail AI chat panel and <code>dicode
                ai</code>. Must have a webhook trigger under <code>/hooks/</code>.
              Defaults to
              <code>buildin/dicodai</code> — point at any ai-agent preset to switch
              providers (e.g. <code>examples/ai-agent-ollama</code>, <code
              >examples/ai-agent-groq</code>).
            </div>
          </div>
          <button class="btn" @click="${() =>
        this._saveAI()}">&#128190; Save AI settings</button>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="cfg-form">
          <div class="field">
            <label>Port</label>
            <input
              .value="${String(srv.Port || srv.port || "")}"
              disabled
              style="color:var(--dicode-muted);cursor:not-allowed"
            >
            <div class="hint">
              Changing port requires restart; edit dicode.yaml directly.
            </div>
          </div>
          <div class="field">
        ...
        false}">Disabled</option>
          ...
          <button class="btn" @click="${() => this._saveServer()}">
      ...
      <div class="card">
        ...
        <table>
          <tbody>
            <tr>
              <th>Type</th>
              <td>${db.Type || db.type || "sqlite"}</td>
            </tr>
            ${db.Path || db.path
        ? html`
          <tr>
            <th>Path</th>
            <td>${db.Path || db.path}</td>
          </tr>
        `
        : ""} ${db.URLEnv || db.url_env
        ? html`
          <tr>
            <th>URL env</th>
            <td>${db.URLEnv || db.url_env}</td>
          </tr>
        `
        : ""}
          </tbody>
      ...
      <div class="card">
        ...
        ? html`
          <tr>
            <td colspan="4" class="meta" style="text-align:center">
        ...
        : sources.map((s) =>
          html`
            <tr>
              <td><span class="badge badge-manual">${s.Type || s.type ||
              ""}</span></td>
              <td style="word-break:break-all">${s.Path || s.path || s.URL ||
              s.url || ""}</td>
              <td class="meta">${(s.Type || s.type) === "git"
              ? `branch: ${s.Branch || s.branch || ""}`
              : `watch: ${s.Watch || s.watch || false}`}</td>
              <td>
                <button
                  class="btn btn-sm"
                  style="background:var(--dicode-red)"
                  @click="${() => this._removeSource(s.name)}"
                >
                  Remove
                </button>
              </td>
        ...
        <details>
          <summary style="cursor:pointer;font-size:0.85rem;color:var(--dicode-lavender);user-select:none">
            + Add source
          </summary>
          <div class="cfg-form" style="margin-top:0.75rem">
            <div class="field">
              ...
              <div class="hint">
                ...
                (e.g. <code>my-source/some-task</code>). Letters, digits, '-', '_', or
                '.' only.
              </div>
            </div>
            <div class="field">
              <label>Type</label>
              <select id="new-src-type" @change="${(e) => {
        this._srcType = e.target.value;
      }}">
        ...
        `}
            <button class="btn" @click="${() =>
        this._addSource()}">Add source</button>
          </div>
        </details>
      </div>
      ...
      <div class="card">
        ...
        <div
          id="config-monaco"
          style="height:400px;border-radius:var(--dicode-radius-sm);overflow:hidden;margin-bottom:0.75rem"
        >
        </div>
        <button class="btn" @click="${() =>
        this._saveRaw()}">&#128190; Save</button>
      </div>
    `;
  }
}
