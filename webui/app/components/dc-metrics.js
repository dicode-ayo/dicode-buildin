...
class DcMetrics extends LitElement {
  ...
  _fmtMB(v) {
    return this._fmt(v, "MB");
  }
  _fmtMs(v) {
    return v != null ? this._fmt(v, "ms") : "—";
  }
  ...
  render() {
    ...
    if (this._error) {
      return html`
        <p style="color:var(--dicode-red)">Error: ${this._error}</p>
      `;
    }
    ...
    return html`
      <div style="display:flex;align-items:center;gap:var(--dicode-space-md);margin-bottom:var(--dicode-space-md);flex-wrap:wrap">
        ...
        <span class="meta">auto-refreshes every ${POLL_INTERVAL_MS /
        1000}s</span>
        ${this._lastUpdated
        ? html`
          <span class="meta" style="margin-left:auto;font-variant-numeric:tabular-nums">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:var(--dicode-space-md)">
        ...
        : this._row("Concurrency cap", "unlimited")} ${tasks.waiting_tasks > 0
        ...
        <div class="card">
          ...
          <table style="width:100%">
            <tbody>
              ${this._row("Goroutines", daemon.goroutines)} ${this._row(
        "Heap alloc",
        this._fmtMB(daemon.heap_alloc_mb),
      )} ${this._row(
        "Heap sys",
        this._fmtMB(daemon.heap_sys_mb),
      )} ${daemon.cpu_ms != null
        ...
        ? html`
          <div class="card">
            ...
            ? this._row(
              "Write p50",
              this._fmtMs(this._data.db.write_latency_p50_ms),
            )
            : ""} ${this._data.db.write_latency_p99_ms != null
            ? this._row(
              "Write p99",
              this._fmtMs(this._data.db.write_latency_p99_ms),
            )
