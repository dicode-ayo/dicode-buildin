...
class DcRelayStatus extends LitElement {
  static styles = css`
    ...
    .pill.off {
      background: rgba(140, 150, 163, 0.10);
      color: #8c96a3;
      border-color: rgba(140, 150, 163, 0.28);
    }
    .dot {
      width: 0.55rem;
      height: 0.55rem;
  ...
  render() {
    ...
    if (!s) {
      return html`
        <span class="pill off" title="loading relay status…"><span
            class="dot"
          ></span>Relay: …</span>
      `;
    }
    if (!s.enabled) {
      return html`
        <span
          class="pill off"
          title="relay disabled in config; set relay.enabled: true in dicode.yaml"
        ><span class="dot"></span>Relay: off</span>
      `;
    }
    const cls = s.connected ? "ok" : "err";
    const label = s.connected ? "Relay" : "Relay offline";
    return html`
      <span class="pill ${cls}" title="${this._tooltip()}">
