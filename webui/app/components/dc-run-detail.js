...
import { fmtDuration, fmtTime } from "../lib/utils.js";
...
class DcRunDetail extends LitElement {
  ...
  async _load() {
    ...
    try {
      ...
      const childrenP = get(
        `/api/runs?parent=${encodeURIComponent(this.runid)}&limit=50`,
      )
        .catch(() => []);
      ...
      if (parentID) {
        try {
          this._parent = await get(`/api/runs/${encodeURIComponent(parentID)}`);
        } catch (_) {
  ...
  _wireWS() {
    ...
    this._offFinished = wsOn("run:finished", (d) => {
      ...
      get(`/api/runs/${this.runid}`).then((run) => {
        this._run = run;
      }).catch(() => {});
    });
  }
  ...
  async _kill() {
    ...
    try {
      await post(`/api/runs/${this.runid}/kill`);
    } catch (e) {
  ...
  _isTextarea(prop) {
    return prop?.type === "string" &&
      (prop.format === "textarea" || prop.format === "multiline");
  }
  ...
  async _submitResume(e) {
    ...
    for (const [name, prop] of Object.entries(props)) {
      ...
      if (
        this._isRequired(name) && (v === "" || v === null || v === undefined)
      ) {
  ...
  _renderResumeField(name, prop) {
    const req = this._isRequired(name)
      ? html`
        <span style="color:var(--dicode-red)"> *</span>
      `
      : "";
    const label = prop.title || name;
    const desc = prop.description
      ? html`
        <span class="meta" style="display:block;font-weight:normal">${prop
          .description}</span>
      `
      : "";
    let input;
    if (prop.type === "boolean") {
      return html`
        <div style="margin-bottom:var(--dicode-space-md)">
          <label><input type="checkbox" name="${name}" ?checked="${prop
          .default === true}">
            ${label}${req}</label>
          ${desc}
        </div>
      ...
      input = html`
        <select name="${name}" style="width:100%">
          ${prop.enum.map((o, i) =>
          html`
            <option value="${i}" ?selected="${o === prop.default}">${String(
              o,
            )}</option>
          `
        )}
    ...
    return html`
      <div style="margin-bottom:var(--dicode-space-md)">
        <label
          style="display:block;font-weight:var(--dicode-font-semibold);margin-bottom:.25rem"
        >${label}${req}</label>
        ${desc} ${input}
      </div>
    `;
  ...
  _renderResumeForm() {
    ...
    return html`
      <h2 style="margin-top:var(--dicode-space-lg)">${schema.title ||
        "Waiting on your input"}</h2>
      <div class="card">
        ${schema.description
        ? html`
          <p class="meta" style="margin-top:0">${schema.description}</p>
        `
        : ""}
        <form @submit="${(e) => this._submitResume(e)}">
          ${props.map(([name, prop]) =>
        this._renderResumeField(name, prop)
      )} ${this._resumeError
        ? html`
          <p style="color:var(--dicode-red)">${this._resumeError}</p>
        `
        : ""}
          <div style="margin-top:var(--dicode-space-md)">
            <button class="btn" type="submit" ?disabled="${this._resuming}">
              ${this._resuming ? "Resuming…" : "Submit"}
            </button>
          </div>
        </form>
      </div>
  ...
  render() {
    ...
    if (retval) {
      try {
        displayRV = JSON.stringify(JSON.parse(retval), null, 2);
      } catch {
        // Not JSON — show the return value verbatim.
      }
    }
    ...
    const parentTask = parent?.task_name || parent?.TaskName ||
      parent?.task_id || parent?.TaskID;
    ...
    return html`
      ...
      ${parent
        ? html`
          <div
            class="card"
            style="margin-bottom:var(--dicode-space-md);display:flex;gap:var(--dicode-space-md);align-items:center"
          >
            <span class="meta">Parent run</span>
            <a href="runs/${parent.ID || parent.id}">
              ${parentTask
            ? `${parentTask} · `
            : ""}<code>${(parent.ID || parent.id || "").slice(0, 8)}</code>
            </a>
          </div>
        `
        : ""}
      ...
      <div class="card">
        ...
        (finishedAt
          ...
          ${isRunning
        ? html`
          <button class="btn" style="background:var(--dicode-red)" @click="${() =>
            this._kill()}">
            Kill
          </button>
        `
        ...
        ? html`
          <button
            class="btn btn-sm"
            @click="${() => this._replay()}"
            title="Re-fire this run with its persisted input"
          >
            Replay
          </button>
        `
      ...
      ${status === "suspended"
        ...
        : ""} ${children.length
        ? html`
          <h2 style="margin-top:var(--dicode-space-lg)">
            Sub-runs <span class="meta">(${children.length})</span>
          </h2>
          <table>
            <thead>
              <tr>
            ...
            html`
              <tr>
                <td><a href="runs/${c.ID}">${(c.TaskID || "?")} · <code>${c.ID
                .slice(0, 8)}</code></a></td>
                <td><span class="badge badge-${c.Status}">${c
                .Status}</span></td>
            ...
            ? html`
              <iframe
                ...
                e.target.style.height =
                  (e.target.contentDocument.body.scrollHeight + 32) + "px";
              }}"
              >
              </iframe>
            `
            : html`
              <pre style="margin:0;border-radius:var(--dicode-radius-sm)">${ocontent}</pre>
            `}
          </div>
        `
        : retval
