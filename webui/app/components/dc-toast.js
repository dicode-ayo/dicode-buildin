...
class DcToast extends LitElement {
  ...
  connectedCallback() {
    ...
    globalThis.addEventListener("dc-toast", this._onToast);
  }

  disconnectedCallback() {
    super.disconnectedCallback();
    globalThis.removeEventListener("dc-toast", this._onToast);
  }
      ...
      ${this._toasts.map((t) =>
        html`
          <div class="${`toast ${t.kind}`}" role="status">
            ...
            <button type="button" aria-label="Dismiss" @click="${() =>
            this._dismiss(t.id)}">×</button>
          </div>
        `
      )}
    `;
